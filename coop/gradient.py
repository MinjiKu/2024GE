...
#     # return torch.tensor([tau_dict[i][j][industry] for i in tau_dict.keys() if i != j for industry in var.industries])
#     # Flatten the dictionary to a tensor, excluding cases where i == j
#     flat_list = []
#     for i in tau_dict:
#         for industry in tau_dict[i]:
...
#     tau_tensor = torch.tensor(initial_tau, requires_grad=True)
...
#         optimizer.zero_grad()
#         loss = gov_obj(tau_tensor, j)  # Calculate the loss (negative objective)
        
#         # Add constraint penalties here (if necessary)
#         # For example: penalty = penalty_weight * constraint_function()
#         # loss += penalty
