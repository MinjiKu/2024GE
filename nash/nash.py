...
# Define p_is, P_j, tau, sigma, t, w, and T as in your original code
...
# Helper functions (fill_gamma_denom, fill_gamma, fill_pi, fill_alpha_denom, fill_alpha)
...
#             if j != country:
#                 tau_copy[country][j][industry] = tau_js[i * (num_countries - 1) + k]
#     total = 0
#     for s in industries:
...
def gov_obj(tau_js, j):
    ...
    for industry in industries:
        for country in countries:
            if country != j:
                tau_copy[country][j][industry] = tau_js[idx]
                idx += 1
    total = 0
    for s in industries:
        total += pol_econ[j][s] * welfare(j, s)
...
# Constraint functions (eq_12, x2, wL, complicated, term3, eq_13, eq_10)
# ... (copy these from your original code)
...
for iteration in range(max_iterations):
    old_tariffs = {i: {j: {industry: nash_tariffs[i][j][industry] for industry in industries} for j in countries if j != i} for i in countries}
    optimal_taus = {j: {industry: 0 for industry in industries} for j in countries}

    for i in countries:     
        for j in countries:
            if j == i:
                continue
            # initial_tau_js = [nash_tariffs[k][j][industry] for industry in industries for k in countries if k != j]
            initial_tau_js = [nash_tariffs[k][j][industry] for industry in industries for k in countries if k != j]
            ...
            #     if k != j:
            #         for industry in industries:
            #             optimal_taus[k][industry] = result.x[idx]
            #             idx += 1
            result = minimize(gov_obj, initial_tau_js, args=(j,), constraints=constraints(initial_tau_js, j))
            idx = 0
            for industry in industries:
                for country in countries:
                    if country != j:
                        optimal_taus[country][industry] = result.x[idx]
                        idx += 1
        
        # Update nash_tariffs for country i
        for j in countries:
            if j != i:
                for industry in industries:
                    nash_tariffs[i][j][industry] = optimal_taus[j][industry]
    
    # Check for convergence
    max_change = max(abs(nash_tariffs[i][j][industry] - old_tariffs[i][j][industry]) 
                     for i in countries for j in countries if j != i for industry in industries)
...
# Print final Nash equilibrium tariffs
for i in countries:
    print(f"Nash equilibrium tariffs for {i}:")
    print(pd.DataFrame(nash_tariffs[i]))
    print("\n")