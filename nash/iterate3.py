...
def calc_x(j, s):
    sum = 0
    sum += w[j] * L_js[j][s] + pi[j][s] + L_js[j][s]/L_j[j] * alpha_denom[j][s]
    return sum


# Welfare function
def welfare(j, s):
    return calc_x(j, s) + P_j[j]
...
def eq_12(j, s):
    ...
    for i in countries:
        if i != j:
            total += (gamma[i][j][s] * (tau_hat[i][j][s] ** (1 - sigma[s]))) ** (1 / (1 - sigma[s]))
    P_hat[j][s] = total
    return total
        ...
        for s in industries:
            if i != j:
                total += (t[i][j][s] * T[i][j][s] / x2(j) * t_hat[i][j][s] * 
                        (eq_12(j, s) ** (sigma[s] - 1)) * (abs(tau_hat[i][j][s]) ** -sigma[s]) + (pi[j][s] / x2(j) * pi_hat[j][s]))
    return total
...
def term3(j):
    ...
    for s in industries:
        for i in countries:
            if i != j:
                total += (pi[j][s] / x2(j) * pi_hat[j][s]) * alpha[j][i][s] * (abs(tau_hat[j][i][s]) ** -sigma[s]) * (w[i] ** (1 - sigma[s])) * (eq_12(i, s) ** (sigma[s] - 1))
    return total
    ...
    for j in countries:
        if i != j:
            total += (alpha[i][j][s] * (tau_hat[i][j][s] ** -sigma[s]) * 
                      (w[i] ** (1 - sigma[s])) * (P_hat[j][s] ** (sigma[s] - 1)) * X_hat[j])
    
    return total - pi_hat[i][s]
...
def update_hats(tau, t, pi): #갱신된 값이 인자로 들어감
    ...
    for i in countries:
        for j in countries:
            if i != j:
                for s in industries:
                    tau_hat[i][j][s] = abs(tau[i][j][s] / factual_tau[i][j][s])
                    t_hat[i][j][s] = abs(t[i][j][s] / factual_t[i][j][s])
    for j in countries:
        for s in industries:
